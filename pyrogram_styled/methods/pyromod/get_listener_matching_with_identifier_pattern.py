...
class GetListenerMatchingWithIdentifierPattern:
    def get_listener_matching_with_identifier_pattern(
        self: pyrogram_styled.Client, pattern: Identifier, listener_type: ListenerTypes
    ) -> Listener | None:
        """
        Gets a listener that matches the given identifier pattern.

        The difference from :meth:`pyrogram_styled.Client.get_listener_matching_with_data` is that this method
        ...
        Returns:
            :obj:`~pyrogram_styled.types.Listener`: The listener that matches the given identifier pattern or ``None`` if no listener matches.
        """
        matching = [
            listener
