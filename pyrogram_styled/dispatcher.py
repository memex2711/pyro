...
from pyrogram_styled import utils, types, raw
...
class Dispatcher:
    ...
    def __init__(self, client: "pyrogram_styled.Client"):
        self.client = client
        ...
        async def message_parser(update, users, chats):
            return (
                await pyrogram_styled.types.Message._parse(self.client, update.message, users, chats, None,
        ...
        async def callback_query_parser(update, users, chats):
            return (
                await pyrogram_styled.types.CallbackQuery._parse(self.client, update, users, chats),
                CallbackQueryHandler
            )

        async def user_status_parser(update, users, chats):
            return (
                pyrogram_styled.types.User._parse_user_status(self.client, update),
                UserStatusHandler
            )

        async def inline_query_parser(update, users, chats):
            return (
                pyrogram_styled.types.InlineQuery._parse(self.client, update, users),
                InlineQueryHandler
            )

        async def poll_parser(update, users, chats):
            return (
                await pyrogram_styled.types.Poll._parse_update(self.client, update, users, chats),
                PollHandler
            )
        ...
        async def chosen_inline_result_parser(update, users, chats):
            return (
                pyrogram_styled.types.ChosenInlineResult._parse(self.client, update, users),
                ChosenInlineResultHandler
            )

        async def chat_member_updated_parser(update, users, chats):
            return (
                pyrogram_styled.types.ChatMemberUpdated._parse(self.client, update, users, chats),
                ChatMemberUpdatedHandler
            )

        async def chat_join_request_parser(update, users, chats):
            return (
                pyrogram_styled.types.ChatJoinRequest._parse(self.client, update, users, chats),
                ChatJoinRequestHandler
            )

        async def story_parser(update, users, chats):
            return (
                await pyrogram_styled.types.Story._parse(self.client, update.story, users, chats, update.peer),
                StoryHandler
            )
    ...
    async def handler_worker(self, lock: asyncio.Lock):
        while True:
            ...
            except pyrogram_styled.StopPropagation:
                pass
        ...
        for group in self.groups.values():
            for handler in group:
                ...
                try:
                    ...
                    raise
                except pyrogram_styled.ContinuePropagation:
                    continue
